#!/usr/bin/env python3
"""Принудительный сброс локальных изменений и обновление кода"""
...
def main():
    ...
    print("\n1️⃣  Сбрасываю локальные изменения...")
    output, _ = run_ssh_command(f"cd {PROJECT_DIR} && git reset --hard HEAD")
    print(output)
    ...
    # 3. Перезапускаем Docker Compose с пересборкой
    print("\n3️⃣  Перезапускаю Docker Compose с пересборкой...")
