#!/usr/bin/env python3
"""
Универсальный парсер накладных для PDF и изображений.

Архитектура:
1. Улучшение изображения (4x разрешение, контраст, яркость, резкость)
2. Google Vision API для OCR
3. OpenAI GPT-4o для извлечения данных (2 промпта: header + items)
4. Экспорт в Excel (путь из .env['XLS_PATH'])

Поддержка:
- Многостраничные PDF
- Одностраничные изображения (JPG, PNG)
- Автоматическая нормализация OCR ошибок

Промпты:
- prompt-header.txt - реквизиты поставщика
- prompt-items_multiple_vs_openai.txt - товары с правилами склейки артикулов

Использование:
  python3 invoice_parser.py invoices/*.pdf
  python3 invoice_parser.py invoices/*.jpg
  python3 invoice_parser.py file.pdf custom_output.xls
"""
...
def parse_items_with_openai(vision_text: str) -> list:
    ...
    Использует промпт из prompt-items_multiple_vs_openai.txt.
    """
    api_key = os.getenv('OPENAI_API_KEY')
    if not api_key:
        raise ValueError("OPENAI_API_KEY не установлен в .env")
    ...
    with open('prompt-items_multiple_vs_openai.txt', 'r', encoding='utf-8') as f:
        system_prompt = f.read()
...
def process_invoice(pdf_path: str, excel_file: str = None):
    """
    Обрабатывает один инвойс: парсит и экспортирует в Excel.
    """
    if excel_file is None:
        excel_file = os.getenv('XLS_PATH', "таблиця накладних.xls")
...
def process_multiple_invoices(pdf_paths: list, excel_file: str = None):
    """
...
def main():
    if len(sys.argv) < 2:
        print("Универсальный парсер накладных для PDF и изображений")
        print("\nИспользование:")
        print("  python3 invoice_parser.py <путь_к_файлу> [output.xls]")
        print("  python3 invoice_parser.py <файл1> <файл2> ... [output.xls]")
        print("  python3 invoice_parser.py invoices/*.pdf")
        print("  python3 invoice_parser.py invoices/*.jpg")
        print("\nПоддерживаемые форматы: PDF, JPG, PNG")
        print("Путь к Excel: .env['XLS_PATH'] или аргумент")
        sys.exit(1)

    # Определяем выходной файл Excel из .env или используем по умолчанию
    excel_file = os.getenv('XLS_PATH', "таблиця накладних.xls")
    pdf_paths = []
