...
class InvoiceDataTransformer:
    ...
    # Maps common field name variants to normalized keys
    # Note: Since column_mapping is dynamically generated by LLM per document,
    # we only map common English normalized keys that may appear
    FIELD_MAPPINGS = {
        'product_name': 'name',
        'product': 'name',
        'no': 'line_number',
    ...
    # Fields that may contain product name (common English normalized keys only)
    POSSIBLE_NAME_FIELDS = ['product_name', 'product', 'name']
    ...
    def map_item_fields(item: Dict[str, Any], column_mapping: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        ...
        if 'name' not in mapped:
            ...
            # If still no name found, use empty string
            if 'name' not in mapped:
                mapped['name'] = ''
