...
from .excel_formatter import ExcelFormatter, DOCUMENT_TYPE_MAPPING
...
class ExcelExporter:
    ...
    def _export_structured(self, wb: Workbook, data: Dict[str, Any]):
        ...
        # Используем общий форматтер
        formatted_rows = ExcelFormatter.format_header_data(data)

        row = 1
        for row_type, *row_data in formatted_rows:
            if row_type == 'SECTION':
                # Секция - объединяем ячейки
                ws1.merge_cells(f'A{row}:B{row}')
                ws1[f'A{row}'] = row_data[0]
                ...
                if len(row_data) == 1:
                    ...
                    # Вторая колонка заголовка
                    ws1[f'B{row}'] = row_data[0] if row_data else self.config.excel_header_value_column
        ...
        headers, rows = ExcelFormatter.format_items_data(data)

        if headers and rows:
            # Заголовки
            for col_idx, header_text in enumerate(headers, start=1):
                ws2.cell(row=1, column=col_idx, value=header_text)
